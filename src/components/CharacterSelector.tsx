...
export function CharacterSelector({ personas, selectedId, onSelect, heroImage }: CharacterSelectorProps) {
  const activePersona = personas.find((p) => p.id === selectedId) ?? personas[0]

  return (
    <section className="card character-section" aria-label="Persona selector">
      ...
      <div className="hero-right">
        <div className="hero-card">
          ...
          <div className="hero-actions">
            <button className="btn-primary" onClick={() => onSelect(activePersona.id)}>
              Meet {activePersona.name}
            </button>
            <button className="btn-secondary" onClick={() => onSelect(activePersona.id)}>
              Talk to {activePersona.name}
            </button>
            <button className="btn-ghost" onClick={() => onSelect(activePersona.id)}>
